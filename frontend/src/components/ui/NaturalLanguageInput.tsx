    ...
    <form onSubmit={handleSubmit} className={clsx('w-full flex flex-col gap-space-xs', className)}>
      ...
      <div className="relative flex items-center border-b-2 border-ink-primary transition-colors focus-within:border-accent-terracotta">
        ...
        <button
          type="submit"
          disabled={!value.trim()}
          className="absolute right-0 text-ink-primary hover:text-accent-terracotta font-label-lg text-label-lg font-semibold uppercase tracking-wider transition-colors disabled:opacity-30 disabled:hover:text-ink-primary"
        >
          Parse &amp; Anchor →
        </button>
      </div>

      <div className="flex items-center justify-between text-ink-muted font-body-md text-body-md pt-space-xs">
